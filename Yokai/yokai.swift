...
protocol YokaiProtocol {
  ...
  mutating func deplacer(y: YokaiProtocol, p: PositonProtocol)->YokaiProtocol

  //deplacerReserve : Yokai -> Yokai
  //Deplace le yokai du joueur dans la réserve
  //Post : L'attribut positon du yokai == nil
  mutating func deplacerReserve(y: YokaiProtocol)->YokaiProtocol
  ...
  func peutAller(y: YokaiProtocol, coord_x: Int, coord_y: Int)->Bool

  //makeItDeplacement: Yokai -> IteratorProtocol
  //Iterator sur les déplacement du Yokai passé en paramètre
  func makeItDeplacement(y: YokaiProtocol)->DeplacementIterator
